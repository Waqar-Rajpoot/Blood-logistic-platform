...
export default function DonorNavbar() {
  ...
  const mainNavItems = [
    { name: "Dashboard", href: "/donor", icon: <LayoutDashboard size={20} /> },
    { name: "Alerts", href: "/donor/alerts", icon: <Bell size={20} /> },
    { name: "Donate Blood", href: "/donor/volunteer", icon: <HeartHandshake size={20} />, special: true },
  ];
    ...
    <nav className="bg-white border-b border-gray-100 sticky top-0 z-50">
        ...
        <div className="flex justify-between h-20">
          ...
          <div className="flex items-center">
            <Link href="/donor" className="flex items-center gap-2 group">
              ...
              <span className="text-red-600 font-medium text-sm mt-3 tracking-tight rounded">Donor</span>
            </Link>
          </div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-100 rounded-2xl shadow-xl py-2 animate-in fade-in zoom-in-95 duration-100">
                  ...
                  >
                    <UserCircle size={20} /> Upate Profile
                  </Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 p-4 space-y-1 shadow-2xl">
          {[...mainNavItems, { name: "History", href: "/donor/history", icon: <History size={18} /> }, { name: "Profile", href: "/donor/profile", icon: <UserCircle size={20} /> }].map((item) => (
            <Link
              key={item.name}
              href={item.href}
