...
export default function ReceiverMainPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const [requestsRes, donorCountRes, userRes] = await Promise.all([
    ...
    <div className="p-6 md:p-10 max-w-8xl mx-auto space-y-10">
      ...
      <div className="bg-gray-900 rounded-[3rem] p-8 md:p-12 text-white flex flex-col md:flex-row justify-between items-center gap-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4 z-10 w-full md:w-auto">
          {[
            { label: "Active Donors", val: stats.totalDonors, icon: <Users className="text-blue-400" />, bg: "bg-blue-500/10" },
            { label: "Units Secured", val: stats.unitsFulfilled, icon: <CheckCircle2 className="text-green-400" />, bg: "bg-green-500/10" },
            { label: "Pending", val: stats.activeRequests, icon: <Clock className="text-orange-400" />, bg: "bg-orange-500/10" },
            { label: "Needs Review", val: stats.pendingVerification, icon: <Activity className="text-red-400" />, bg: "bg-red-500/10" },
          ].map((item, i) => (
            <div key={i} className={`${item.bg} p-6 rounded-[2.5rem] border border-white/5 text-center min-w-[140px]`}>
              <div className="mb-2 flex justify-center">{item.icon}</div>
              <p className="text-3xl font-black">{loading ? "..." : item.val}</p>
              <p className="text-gray-500 text-[9px] uppercase font-black tracking-tighter">{item.label}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              <div>
                <h3 className="text-2xl font-black text-gray-800 tracking-tight">Active Request Tracker</h3>
                <p className="text-sm text-gray-500 font-medium">Real-time status of your most recent post</p>
              </div>
              <Link href="/receiver/my-requests" className="bg-gray-50 hover:bg-gray-100 p-3 rounded-2xl transition-all">
            ...
            {loading ? (
              <div className="flex items-center justify-center py-12 text-gray-400 gap-3">
                <Loader2 className="animate-spin" /> <span className="font-bold">Syncing live data...</span>
              </div>
            ) : latestRequest ? (
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <p className="font-black text-gray-900 text-xl">
                        {latestRequest.unitsRequired} Units • {latestRequest.patientName}
                      </p>
                      <div className="flex items-center gap-4 mt-2">
                        <span className="text-sm font-bold text-gray-500 flex items-center gap-1">
                          <MapPin size={14} className="text-red-500" /> {latestRequest.city}
                        </span>
                        <span className="text-sm font-bold text-blue-600 flex items-center gap-1">
                          <Users size={14} /> {latestRequest.potentialDonors?.length || 0} Responses
                        </span>
                      </div>
                    </div>
                  </div>
                  <Link href={`/receiver/my-requests`} className="mt-6 md:mt-0 bg-white border-2 border-gray-200 px-6 py-3 rounded-2xl font-black text-sm hover:border-red-600 hover:text-red-600 transition-all">
                    VIEW DETAILS
                  </Link>
                </div>
              </div>
              ...
              <div className="py-16 text-center border-4 border-dashed border-gray-50 rounded-[3rem]">
                ...
                <p className="text-gray-400 font-black text-lg">No active broadcast found.</p>
                <Link href="/receiver/request" className="text-red-600 font-black underline underline-offset-4">Create First Request</Link>
              </div>
            )}
          </section>
              ...
              <div className="space-y-4">
                {["A+", "B+", "O-", "AB+"].map((group, index) => (
                  <div key={group} className="flex items-center gap-4">
                    <span className="text-xs font-black text-gray-400 w-8">{group}</span>
                    <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="bg-red-500 h-full rounded-full transition-all duration-1000"
                        style={{ width: hasMounted ? `${[85, 65, 45, 30][index]}%` : "0%" }}
                      ></div>
                    </div>
                  </div>
                ))}
        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-xl font-black text-gray-900">{receiverProfile?.username || "Receiver"}</h3>
                  <p className="text-sm text-gray-500 font-bold uppercase tracking-wider">{receiverProfile?.role}</p>
                </div>
                <div className="pt-4 border-t border-gray-50 space-y-3">
                  <div className="flex items-center gap-3 text-sm font-bold text-gray-600">
                    <MapPin size={16} className="text-red-500" />
                    {receiverProfile?.city || "Location not set"}
                  </div>
                  <div className="flex items-center gap-3 text-sm font-bold text-gray-600">
          ...
          {/* POLICY CARD */}
          <div className="bg-red-600 p-8 rounded-[3rem] text-white shadow-2xl shadow-red-200">
            <div className="bg-white/20 w-14 h-14 rounded-2xl flex items-center justify-center mb-8">
              <ShieldAlert size={28} />
            </div>
            <h3 className="text-2xl font-black leading-tight mb-4">Critical <br /> Verification Policy</h3>
            <p className="text-red-100/80 text-sm font-medium leading-relaxed mb-6">
              Post-donation verification is mandatory. It resets the donor recovery clock.
            </p>
            <ul className="space-y-4">
              {["Verify within 24 hours", "Report no-show donors"].map((text, i) => (
                <li key={i} className="flex items-start gap-3 text-xs font-bold">
                  <CheckCircle2 size={16} className="text-red-300 shrink-0" /> {text}
                </li>
              ))}
            </ul>
          </div>
