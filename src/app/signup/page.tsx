...
// import {
//   User, Mail, Lock, Phone,
//   Droplets, Briefcase, Eye, EyeOff, Map as MapIcon, AlertCircle
// } from "lucide-react";
// import PasswordStrength from "@/components/PasswordStrength";
// import dynamic from "next/dynamic";
...
//     city: "",
//     area: "",
//     latitude: null as number | null, // Removed default Sahiwal
...
//     // Final check for map pin
//     if (user.latitude === null || user.longitude === null) {
//       setLocationError(true);
//       toast.error("Please pin your exact location on the map");
//       return;
//     }
...
//       const payload = {
//         ...user,
//         // Send empty blood group for receivers
...
//   useEffect(() => {
//     const isLocationPinned = user.latitude !== null && user.longitude !== null;
//     const isBloodGroupValid = user.role === "receiver" ? true : user.bloodGroup.length > 0;
...
//             <div className="space-y-1">
//               <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><User size={16} /> Username</label>
//               <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-red-300 transition-colors" type="text" value={user.username} onChange={(e) => setUser({ ...user, username: e.target.value })} placeholder="johndoe" required />
//             </div>

//             <div className="space-y-1">
//               <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><Mail size={16} /> Email</label>
//               <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-red-300 transition-colors" type="email" value={user.email} onChange={(e) => setUser({ ...user, email: e.target.value })} placeholder="name@email.com" required />
//             </div>

//             <div className="space-y-1">
//               <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><Lock size={16} /> Password</label>
//               <div className="relative">
//                 <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-red-300 transition-colors" type={showPassword ? "text" : "password"} value={user.password} onChange={(e) => setUser({ ...user, password: e.target.value })} placeholder="••••••••" required />
//                 <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">
//                   {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
//                 </button>
//               </div>
...
//             <div className="space-y-1">
//               <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><Briefcase size={16} /> I am a</label>
//               <select className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-red-300 transition-colors cursor-pointer" value={user.role} onChange={(e) => setUser({...user, role: e.target.value})}>
//                 <option value="donor">Donor</option>
//                 <option value="receiver">Receiver / Hospital</option>
//               </select>
//             </div>
...
//                   <span className="flex items-center gap-2"><MapIcon size={16} /> Pin Your Location</span>
//                   {user.latitude && <span className="text-[10px] text-green-600 font-bold uppercase">Location Set</span>}
//                 </label>
//                 <div className={`rounded-2xl overflow-hidden border-2 transition-all ${locationError ? "border-red-500 shadow-lg shadow-red-50" : "border-transparent"}`}>
//                   <MapPicker 
//                     onLocationSelect={handleLocationSelect} 
...
//                     <AlertCircle size={12} /> Please click on the map to pin your location.
//                   </p>
//                 )}
//              </div>
...
//                 <div className="space-y-1">
//                   <label className="text-sm font-bold text-gray-700">City</label>
//                   <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none" type="text" value={user.city} onChange={(e) => setUser({ ...user, city: e.target.value })} placeholder="City" required />
//                 </div>
//                 <div className="space-y-1">
//                   <label className="text-sm font-bold text-gray-700">Area</label>
//                   <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none" type="text" value={user.area} onChange={(e) => setUser({ ...user, area: e.target.value })} placeholder="Area/Street" required />
//                 </div>
//              </div>
//           </div>
...
//                   user.role === "receiver" ? "bg-gray-100 text-gray-400 cursor-not-allowed opacity-60" : "bg-gray-50"
//                 }`}
//                 value={user.role === "receiver" ? "" : user.bloodGroup} 
//                 onChange={(e) => setUser({ ...user, bloodGroup: e.target.value })}
//               >
//                 {user.role === "receiver" ? (
//                   <option value="">Not Required for Hospital</option>
//                 ) : (
//                   <>
//                     <option value="">Select Group</option>
//                     {["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"].map(g => <option key={g} value={g}>{g}</option>)}
...
//               disabled={buttonDisabled || loading}
//               className={`w-full p-4 rounded-2xl font-black text-white transition-all uppercase tracking-widest ${
//                 buttonDisabled || loading ? "bg-gray-300 cursor-not-allowed" : "bg-red-600 hover:bg-red-700 shadow-xl shadow-red-100 active:scale-95"
//               }`}
//             >
//               {loading ? "Registering..." : "Register & Save Lives"}
//             </button>
...
export default function Signup() {
  ...
  const onSignup = async (e: React.FormEvent) => {
    ...
    if (user.latitude === null || user.longitude === null) {
      toast.error("Critical: Map location is required.");
      return;
    }
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-xl w-full max-w-3xl border border-gray-100">
        ...
        <form onSubmit={onSignup} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-sm font-bold text-gray-700 flex items-center gap-2"><Briefcase size={16} /> I am a</label>
              <select className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-red-300 transition-colors cursor-pointer font-bold text-gray-700" value={user.role} onChange={(e) => setUser({...user, role: e.target.value})}>
                <option value="donor">Donor</option>
                <option value="receiver">Receiver / Hospital</option>
              </select>
            </div>
             ...
             <div className="space-y-1">
                <div className="flex items-center justify-between mb-1">
                   ...
                   {user.latitude ? (
                     ...
                     <span className="flex items-center gap-1 text-[10px] text-red-600 font-black uppercase bg-red-50 px-2 py-0.5 rounded-full border border-red-200 animate-pulse">
                       <AlertCircle size={10} /> Pin Required
                     </span>
                   )}
                </div>

                <div className={`rounded-2xl overflow-hidden border-2 transition-all duration-300 ${!user.latitude ? "border-red-100 shadow-inner" : "border-green-100 shadow-sm"}`}>
                  <MapPicker 
                    onLocationSelect={handleLocationSelect} 
                    defaultLocation={user.latitude ? {lat: user.latitude, lng: user.longitude!} : undefined} 
                  />
                ...
                {/* INSTRUCTIONAL NOTE */}
                {!user.latitude && (
                   <p className="text-[11px] text-red-500 font-bold bg-red-50/50 p-2 rounded-lg border border-red-100 mt-2">
                     Notice: You must click the map to pin your location before the register button becomes active.
                   </p>
                )}
             </div>
          ...
          <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-50">
            <div className="space-y-1">
              ...
              <select 
                disabled={user.role === "receiver"}
                className={`w-full p-3 border border-gray-200 rounded-xl outline-none transition-all font-bold ${
                  user.role === "receiver" ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-gray-50 text-gray-700"
                }`}
                value={user.role === "receiver" ? "" : user.bloodGroup} 
                onChange={(e) => setUser({ ...user, bloodGroup: e.target.value })}
              >
          ...
          <div className="md:col-span-2 mt-6">
            ...
            >
              {loading ? "Initializing..." : "Register & Save Lives"}
            </button>
            <p className="text-center text-gray-400 mt-6 text-xs font-bold uppercase tracking-tighter">
